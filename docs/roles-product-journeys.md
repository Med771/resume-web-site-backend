...
В системе четыре роли (`RoleEnum`): **`GUEST`**, **`USER`**, **`STUDENT`**, **`ADMIN`**.

- В JWT и Spring Security ожидаются authorities вида **`ROLE_<ИМЯ>`** (согласовано с `UserHelper` / данными пользователя).
- **`GUEST`** и **`USER`** в API **работодателя** обычно объединены в `hasAnyRole('GUEST', 'USER', 'ADMIN')`: оба могут смотреть каталог (с ограничениями по «курсу» студента, см. ниже), создавать заявки, пользоваться чатом в рамках правил видимости.
- **`STUDENT`** — владелец привязанной карточки студента; **не** создаёт заявки (`POST /request` запрещён).
- **`ADMIN`** — полный доступ к модерации, справочникам, заявкам, пользователям, мягкому удалению сообщений и полной истории чатов в REST.

**Учётные записи для разработки** (создаются при старте, если ещё нет в БД; см. `application.yaml` → `app.user.logins`): демо-пользователи `guest` / `user` / `admin` с паролями из конфига. В продакшене их нужно отключить или сменить.
...
|----------|------|------------|
| Регистрация студента | `POST /auth/register-student` | Создание пользователя с ролью `STUDENT`, карточки студента, выдача **HttpOnly** cookie `ACCESS_TOKEN` / `REFRESH_TOKEN` |
...
| Публичная витрина студентов | `GET /public/students/{id}`, `POST /public/students/cards` | Только карточки с `public_profile_consent = true` и курс **не** `NEW`; без cookie |
...
1. **Карточки:** `POST /public/students/cards` с тем же телом фильтра, что и у рекрутера (`FilterStudentReq` + пагинация `page`/`size`), но сервер дополнительно требует **`public_profile_consent = true`** и исключает курс **`NEW`**. Параметры сортировки — поля **`sortBy`**, **`sortDirection`**, **`useDefaultRanking`** в теле (произвольный `sort=` из query для этих методов не используется).
2. **Деталь:** `GET /public/students/{id}` — **404**, если нет согласия, курс `NEW` или запись скрыта по тем же правилам, что и для не-админов на `GET /student/{id}`.
...
1. Клиент подгружает справочники через `/public/registration/...`.
2. `POST /auth/register-student` с телом `StudentSelfRegistrationReq`: логин, пароль, подтверждение, контакты, специальность, навыки, опционально опыт и образование (с привязкой к справочникам или свободными полями — см. Swagger).
3. Ограничения: лимит попыток с одного IP (`app.registration.rate-limit-per-ip-per-hour`), политика пароля (`min-password-length`, `require-letter-and-digit`), лимиты на число навыков/опыта/образований в одной регистрации.
4. Создаётся карточка студента с **курсом `NEW`** и пользователь с ролью **`STUDENT`**, сразу выдаются те же cookie, что и при логине.
...
- **`POST /request`** от имени такого студента для рекрутера невозможен как создание заявки студентом; рекрутер при попытке заявки на такого студента также получает **404** (скрытие «непроверенной» карточки).

Продуктовый смысл: студент после саморегистрации **может войти в ЛК**, но **не виден рекрутерам**, пока админ не сменит курс/не проведёт модерацию (через админские `PUT`/`PATCH` студента — детали полей в Swagger).
...
1. **Логин** → cookie.
2. **Каталог**: `POST /student/cardsFilter`, `POST /student/filter`, детальная карточка `GET /student/{id}` — студенты с курсом **`NEW`** в выдаче **только у админа**; для рекрутера они отфильтрованы / недоступны по id.
...
| Студенты | полный CRUD, расширенное создание, фото, смена полей включая **курс** (вывод из `NEW` на витрину) |
...
Админ обрабатывает **очередь регистраций работодателей** → выверяет **новых студентов** (`NEW`) и публикует их на витрину → правит справочники → при спорных диалогах **входит в чат** и при необходимости **удаляет** сообщение или **удаляет** заявку → смотрит заявки фильтром `POST /request/filter`.

---

## 8. Сквозной сценарий «рекрутер ↔ студент» (склейка ролей)

1. **Админ** (или процесс) готовит справочники и карточку студента с курсом не `NEW` **или** студент сам регистрируется и ждёт модерации.
...
## 9. Известные ограничения и риски (для честного описания «как в жизни»)
